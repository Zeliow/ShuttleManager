...
public partial class ShuttleHubControlComponent : IAsyncDisposable
{
    ...
    private void ParseAndHandleResponse(string response)
    {
        ...
        else if (line.StartsWith("Forwrd plt dist"))
        {
            ...
            if (match.Success)
            {
                if (int.TryParse(match.Groups[1].Value, out int forwardPalletDistance) &&
                    int.TryParse(match.Groups[2].Value, out int reversePalletDistance))
                {
                    Shuttle.ForwardPalletDistance = forwardPalletDistance;
                    Shuttle.ReversePalletDistance = reversePalletDistance;
                    Logger.LogInformation($"Parsed Pallet Dist F/R: {forwardPalletDistance}/{reversePalletDistance}");
                }
            }
        }
    ...
    {
        var fileLogsBuffer = new List<string>();
        var terminalLogsBuffer = new List<string>();
        ...
        try
        {
            while (!_componentCts.Token.IsCancellationRequested)
            {
                ...
                {
                    fileLogsBuffer.Clear();
                    terminalLogsBuffer.Clear();

                    while (_logChannel.Reader.TryRead(out var logItem))
                    {
                        var (log, timestamp) = logItem;

                        // Process telemetry and status blocks on background thread
                        ...
                        // Prepare terminal messages
                        if (log.Contains("##HEARTBEAT##"))
                        {
                            terminalLogsBuffer.Add($"[HEARTBEAT] {log}\n");
                        }
                        else
                        {
                            var cleanLog = log.Contains("##TELEMETRY##") ? log.Substring(0, log.IndexOf("##TELEMETRY##")) : log;
                            terminalLogsBuffer.Add($"[{timestamp:HH:mm:ss}] {cleanLog}\n");
                        }

                        // Prepare file log lines
                        fileLogsBuffer.Add($"[{timestamp}] {log}");
                    }

                    if (fileLogsBuffer.Count > 0)
                    {
                        // Background file I/O
                        try
                        {
                            await File.AppendAllLinesAsync(pathLogShuttle, fileLogsBuffer, _componentCts.Token);
                        }
                        catch (Exception ex)
                        {
                        ...
                        // Update terminal message collection in bulk
                        Shuttle.AddRangeTerminalMessages(terminalLogsBuffer);
