...
public partial class ShuttleHubControlComponent : IAsyncDisposable
{
    ...
    private void OnLogReceived(string ip, string log)
    {
        ...
        // Decouple network from I/O by writing to the channel with a timestamp.
        ...
        try
        {
            while (!_componentCts.Token.IsCancellationRequested)
            {
                if (await _logChannel.Reader.WaitToReadAsync(_componentCts.Token))
                {
                    ...
                    // Drain the channel to process messages in a single batch.
                    while (_logChannel.Reader.TryRead(out var logItem))
                    {
                        var log = logItem.Message;
                        var timestamp = logItem.Timestamp;

                        fileLogs.Add($"[{timestamp}] {log}");

                        // Status parsing and terminal formatting (outside UI thread)
                        ...
                        else
                        {
                            ...
                            terminalMessages.Add($"[{timestamp:HH:mm:ss}] {cleanLog}\n");
                        }
                    }

                    // Background file I/O to avoid blocking the loop or UI.
                    if (fileLogs.Count > 0)
                    {
                        try
                        {
                        ...
                        {
                            Logger.LogWarning(ex, "Failed to write batch logs to file");
                        }
                    }
