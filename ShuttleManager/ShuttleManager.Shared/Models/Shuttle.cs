...
public class Shuttle
{
    ...
    public void RemoveTerminalMessage()
    {
        lock (_lock)
        {
            PruneTerminalMessagesInternal(true);
        }
    }
    ...
    private void PruneTerminalMessagesInternal(bool force = false)
    {
        const int limit = 900;
        const int target = 500;

        if (force || _terminalMessages.Count > limit)
        {
            int toRemove = _terminalMessages.Count - target;
            if (toRemove > 0)
            {
                _terminalMessages.RemoveRange(0, toRemove);
            }
        }
    }
}